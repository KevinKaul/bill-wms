...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await prisma.$transaction(async (tx: any) => {
      ...
      if (validatedData.type === ProductType.FINISHED_PRODUCT && validatedData.bomItems && validatedData.bomItems.length > 0) {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        reference_purchase_price: result.referencePurchasePrice?.toNumber() || null,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const products = await prisma.product.findMany({
      ...
      skip: ((validatedParams.page || 1) - 1) * (validatedParams.pageSize || 10),
